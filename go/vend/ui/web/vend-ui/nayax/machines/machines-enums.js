...
(function() {
    'use strict';
    window.NayaxMachines = window.NayaxMachines || {};
    ...
    var STATUS_CLASSES = {
        'online': 'layer8d-status-active', 'offline': 'layer8d-status-inactive',
        'warning': 'layer8d-status-pending', 'pending': 'layer8d-status-pending'
    };
    var TYPE_MAP = {
    ...
    NayaxMachines.enums = { STATUS_MAP: STATUS_MAP, TYPE_MAP: TYPE_MAP };
    NayaxMachines.render = {
        machineStatus: function(value) {
            var label = STATUS_MAP[value] || value || 'Unknown';
            var cls = STATUS_CLASSES[value] || '';
            return cls ? '<span class="layer8d-status-badge ' + cls + '">' + label + '</span>' : label;
        },
        machineType: function(value) { return TYPE_MAP[value] || value || 'Unknown'; }
    };
    NayaxMachines.primaryKeys = {
